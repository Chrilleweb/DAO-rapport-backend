  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("edit comment", async (data) => {
      ...
      socket.on("delete report", async ({ reportId, userId }) => {
        try {
          const result = await ReportController.deleteReport({
            reportId,
            userId,
          });
      ...
      // Lyt til sletning af kommentarer
    ...
    socket.on("delete scheduled report comment", async (data) => {
      try {
        const result = await CommentController.deleteScheduledReportComment(
          data
        );
    ...
    socket.on("delete scheduled report", async (data) => {
      try {
        const result = await ReportController.deleteScheduledReport(data);
