  ...
  io.on("connection", async (socket) => {
    ...
    socket.on('schedule report', async (data) => {
      try {
        ...
        await Rapport.createScheduledReport({
          user_id,
          content,
          report_type_id, // Brug det som en enkelt værdi
          scheduled_time,
        });
  ...
  setInterval(async () => {
    try {
      ...
      for (const report of dueReports) {
        ...
        // Sikre at created_at er en gyldig dato og konvertere den
        if (fullReport && fullReport.created_at) {
          fullReport.created_at = convertToUTC(new Date(fullReport.created_at));
        }
  
        // Emit rapporten til klienterne
        io.emit("new report", fullReport);
      }
    } catch (error) {
